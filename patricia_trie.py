...
class PatriciaTrie():
    ...
    def _scan(self, rvalFun, string, start=0, *end):
        node = self
        start, _ = PatriciaTrie.__offsets(len(string), start, None)
    ...
    def __setitem__(self, key, value):
        ...
        while keylen != idx:
            if key[idx] in node._edges:
                ...
                node._edges[key[idx]] = (key[idx:], PatriciaTrie(value))
                break
        else:
            node._value = value
    ...
    def item(self, string, start=0, end=None, default=__NON_TERMINAL__):
        ...
        node = self
        strlen = len(string)
        start, end = PatriciaTrie.__offsets(strlen, start, end)
        idx = start
        last = self._value
        while idx < strlen:
            ...
            if node is None:
                ...
                last = node._value
        return PatriciaTrie.__check(last, string[start:idx], default)
